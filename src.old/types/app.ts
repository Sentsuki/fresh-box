...
export function normalizeAppSettings(settings: AppSettings | null | undefined): AppSettings {
  ...
  return {
    schema_version:
      typeof settings.schema_version === "number" ? settings.schema_version : 2,
    app: {
    ...
    singbox_core: {
      ...
      selected_option_key: settings.singbox_core?.selected_option_key ?? "",
    },
    pages: {
      proxies: {
      ...
      connections: {
        current_tab: CONNECTION_TABS.includes(settings.pages?.connections?.current_tab)
        ...
        visible_columns:
          visibleColumns.length > 0
            ? visibleColumns
            : [...DEFAULT_CONNECTION_VISIBLE_COLUMNS],
        sort_key: CONNECTION_COLUMNS.has(settings.pages?.connections?.sort_key)
        ...
        grouped_column: CONNECTION_COLUMNS.has(
          settings.pages?.connections?.grouped_column as ConnectionColumnKey,
        )
