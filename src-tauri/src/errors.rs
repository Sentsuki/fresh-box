...
pub enum CommandError {
    ...
    #[error("{0}")]
    NetworkError(String),
    /// A declined UAC prompt (`daemon::install::run_elevated`), distinct
    /// from a genuine install/uninstall/start failure — matching the
    /// official client's `EXIT_CODE_CANCELLED` handling in `repair.ts`. The
    /// frontend should treat this as "the user changed their mind", not as
